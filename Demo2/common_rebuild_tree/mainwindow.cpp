...
void MainWindow::on_pushButton_clicked()
{
    ...
    string tmp;
    string pr_st = pr_s.toStdString();
    string in_st = in_s.toStdString();
    string post_st = af_s.toStdString();
    for (int i = 0; i < (int)pr_st.length(); i++)
    {
        if (pr_st[i] == ',')
        {
            preorder.push_back(atoi(tmp.c_str()));   //*char to int
            tmp = "";
        }
        else
        {
    ...
    for (int i = 0; i < (int)in_st.length(); i++)
    {
        if (in_st[i] == ',')
        {
            inorder.push_back(atoi(tmp.c_str()));   //*char to int
            tmp = "";
        }
        else
        {
